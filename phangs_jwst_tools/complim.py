...
def fake_image(test_hdu, test_psf, n_sources, charflux=100 * u.uJy, return_hdu=False, fluxspread=2):
    """
    ...
    n_sources (int): Number of sources to generate. Note only sources inside the valid regions of the image will be generated.
    charflux (Quantity, optional): Characteristic flux of the sources. Defaults to 100 uJy.
    return_hdu (bool, optional): If True, return an HDU list instead of an image. Defaults to False.
    ...
    keep = keep & ((xfake - psfshape[1]//2 > 0) 
    ...
    for ii in np.arange(len(fluxfake)):
        ...
        psf_shift = map_coordinates(test_psf[0].data, [yarr - (yfrac) * oversample - oversample/2, 
                                                    xarr - (xfrac) * oversample - oversample/2], 
                                    order=1, mode='constant', cval=0.0) 
        # group into blocks of oversamp x oversamp
        psf_shift = np.add.reduceat(psf_shift, 
                                    np.arange(0, psf_shift.shape[0], oversample), 
                                    axis=0)
        psf_shift = np.add.reduceat(psf_shift,
                                    np.arange(0, psf_shift.shape[1], oversample),
                                    axis=1)
        psf_shift *= (fluxconv.value * fluxfake[ii].value)
        fakeimage[int(yy) - psf_shift.shape[0]//2:int(yy) + psf_shift.shape[0]//2 + psf_shift.shape[0] % 2,
                  int(xx) - psf_shift.shape[1]//2:int(xx) + psf_shift.shape[1]//2 + psf_shift.shape[1] % 2] += psf_shift
    if return_hdu:
        hdulist = fits.HDUList([fits.PrimaryHDU(fakeimage, header=test_hdu[1].header), fits.BinTableHDU(fakecatalog)])
...
def constrained_diffusion(inputdata, err_rel=3e-2, n_scales=None):
    ...
    ntot = min(int(np.log(min(data.shape)) / np.log(2) - 1), 
               n_scales or float('inf'))
    ...
    for i in range(ntot):
        ...
        for _ in range(niter):
            ...
            sm_image_min, sm_image_max = (np.minimum(data, smooth_image), 
                                          np.maximum(data, smooth_image))
            diff_image = np.zeros_like(data)
            pos_1, pos_2 = (np.where((data - sm_image_min > 0) & (data > 0)),
                            np.where((data - sm_image_max < 0) & (data < 0)))
            diff_image[pos_1], diff_image[pos_2] = (data[pos_1] - sm_image_min[pos_1],
...
def phot_catalog(image, wcs, psf_fwhm, xcentroids=None, ycentroids=None, filter_diffuse=False):
    """
    ...
    xcentroids (ndarray, optional): X coordinates of the sources. If None, they will be computed. Defaults to None.
    ycentroids (ndarray, optional): Y coordinates of the sources. If None, they will be computed. Defaults to None.
    filter_diffuse (bool, optional): If True, apply a diffusion filter to the image. Defaults to False.

    Returns:
    Table: Photometric catalog with columns for xcentroid, ycentroid, aperflux (aperture flux), bkgflux (background flux), RA, DEC, and skycoord.
    """
    if xcentroids is not None and ycentroids is not None:
        ...
        # # diffuse = np.sum(filtered_data[0][pivot_scale:], axis=0) + filtered_data[1]
            ...
            compact = image
        std = mad_std(compact[compact != 0])
        iraffind = IRAFStarFinder(fwhm=2.0, threshold=5.*std, minsep_fwhm=0.0)
        sources = iraffind(compact)
    aper = CircularAperture(np.c_[sources['xcentroid'], sources['ycentroid']], r=psf_fwhm * 2.5)
    bkaper = CircularAnnulus(np.c_[sources['xcentroid'], sources['ycentroid']], 
                            r_in=psf_fwhm*4, r_out=psf_fwhm*5)
...
def completeness_analysis(fakefiles, catfiles, compfrac=0.99, verbose=True, match_radius=0.1 * u.arcsec):
    """
    Given a list of sources with fakes injected and a list of catalogs for those sources, measure the completeness limit.
    """
    sourcecats = []
    for thisfile, catfile in zip(fakefiles, catfiles):
        ...
        if 'skycoord' not in fake_catalog.keys():
            fake_catalog['skycoord'] = SkyCoord(ra=fake_catalog['RA'], 
                                                dec=fake_catalog['DEC'], 
                                                unit=(u.deg, u.deg))
        if 'skycoord' not in true_fakes.keys():
            true_fakes['skycoord'] = SkyCoord(ra=true_fakes['RA'], 
                                              dec=true_fakes['DEC'], 
                                              unit=(u.deg, u.deg))
        _, sep2d, _ = match_coordinates_sky(true_fakes['skycoord'],
                                            fake_catalog['skycoord'])
    ...
    complim = 1e1**(-(np.log(1/compfrac - 1) + compmodel.params['Intercept']) / compmodel.params['LOGFLUX'])
    if verbose:
        ...
        ax.plot(x, 1/(1 + np.exp(-compmodel.params['Intercept'] 
                                 -compmodel.params['LOGFLUX']*np.log10(x))), color='r')
        ax.set_xscale('log')
...
def completeness_limit(FILENAME,
                       oversample=4,
    ...
    match_radius (Quantity, optional): Maximum distance for a source to be considered a match. Defaults to 0.1 arcsec.
    n_batch (int, optional): Number of batches to process. Defaults to 5.
    n_sources (int, optional): Number of sources to generate in the fake image. Defaults to 1000.
    write_fake_files (bool, optional): If True, write images with fake sources. Defaults to False.
    write_catalogs (bool, optional): If True, write catalogs of the fake sources. Defaults to False.
    compfrac (float, optional): Completeness fraction to calculate the limit for. Defaults to 0.99.
    verbose (bool, optional): If True, print additional information and plot the completeness curve. Defaults to True.
    ...
    psf_fwhm = 2 * np.sqrt(np.sum(test_psf[0].data >= (0.5 * test_psf[0].data.max()))/np.pi)
    sourcecats = []
    for batches in range(n_batch):
        fakeimage, true_fakes = fake_image(test_image, test_psf, n_sources, return_hdu=write_fake_files)
        if write_fake_files:
            outfile = os.path.basename(FILENAME.replace('.fits', f'_fake_{batches}.fits'))
            fakeimage.writeto(outfile, overwrite=True)
            fakeimage = fakeimage[0].data
        fake_catalog = phot_catalog(fakeimage, w, psf_fwhm)
        if write_catalogs:
            fake_catalog.write(os.path.basename(FILENAME.replace('.fits', f'_fakecat_{batches}.fits')), format='fits')
        truefakes_catalog = phot_catalog(fakeimage, w, psf_fwhm,
                                        xcentroids=true_fakes['xcentroid'],
                                        ycentroids=true_fakes['ycentroid'])
        match_idx, sep2d, _ = match_coordinates_sky(truefakes_catalog['skycoord'],
                                                    fake_catalog['skycoord'])
        matched = sep2d < match_radius
        ...
        truefakes_catalog['MEASURED_FLUX'][matched] =  (fake_catalog['aperflux'][match_idx[matched]] 
                                                        - fake_catalog['bkgflux'][match_idx[matched]]) / fluxconv
        truefakes_catalog['FLUX_AT_LOCATION'] = (truefakes_catalog['aperflux'] - truefakes_catalog['bkgflux'])/ fluxconv
...
if __name__ == '__main__':
    FILENAME = sys.argv[1]
    complim, _ = completeness_limit(FILENAME, n_sources=1000, write_fake_files=True, 
                                    write_catalogs=True, compfrac=0.99)
